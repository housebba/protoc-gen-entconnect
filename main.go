...
// Modifications made by housebba on 2025-01-21:
// - 2025-01-21: Created a Connect plugin.
	...
	tmpl, err := gen.NewTemplate("service").
		Funcs(template.FuncMap{
			...
			"statusErr": func(code, msg string) string {
				return fmt.Sprintf("%s(%s, fmt.Errorf(%q))",
			...
			"statusErrf": func(code, format string, args ...string) string {
				return fmt.Sprintf("%s(%s, fmt.Errorf(%s, %s))",
					g.QualifiedGoIdent(connectPackage.Ident("NewError")),
					g.QualifiedGoIdent(codes.Ident(code)),
